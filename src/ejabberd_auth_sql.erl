...
do_check_password(false,_User, _AuthzId, _Server, _Password) ->
    {false, <<"frozen-in">>};
do_check_password(_,User, AuthzId, Server, Password) ->
...
check_plain_password(Server,User,Password) ->
    try sql_queries:get_password(Server, User) of
        {selected,_, [[Password]]} ->
            Password /= <<"">>;
        {selected, _,[[_Password2]]} ->
