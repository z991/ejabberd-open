...
register_route(Domain, ServerHost, LocalHint) ->
    case {jid:nameprep(Domain), jid:nameprep(ServerHost)} of
      ...
      {LDomain, LServerHost} ->
	?DEBUG("LDomain ~p,LServerHost ~p ~n",[LDomain, LServerHost]),
	  Pid = self(),
...
unregister_route(Domain) ->
	?DEBUG("unregister_route domain ~p, ~p ~n",[Domain,self()]),
...
unregister_routes(Domains) ->
	?DEBUG("unregister_route domains ~p, ~p ~n",[Domains,self()]),
...
handle_info({'DOWN', _Ref, Type, Pid, Info}, State) ->
    ...
    F = fun () ->
		Es = mnesia:select(route,
				   [{#route{pid = Pid, _ = '_'}, [], ['$_']}]),
                ?DEBUG("the delete object is ~p~n", [Es]),
		lists:foreach(fun (E) ->
...
do_route(OrigFrom, OrigTo, OrigPacket) ->
	...
	of
	  ...
	  case mnesia:dirty_read(route, LDstDomain) of
	    ...
	    [R] ->
		Pid = R#route.pid,
		case catch check_local_host_limit(From#jid.lserver,To#jid.lserver) of
		true ->
			if node(Pid) == node() ->
...
check_local_host_limit(FHost,THost) ->
   case FHost =:= <<"ejabhost2">> orelse FHost =:= THost orelse str:str(FHost,<<"conference">>) =/= 0  orelse str:str(THost,<<"conference">>) =/= 0  of
   true ->
        true;
   _ ->
        false
