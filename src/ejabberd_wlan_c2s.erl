...
-define(STREAM_KICK_ERRORT(Condition, Cdata,Code, Lang, Text),
    #xmlel{name = <<"stream:error">>, attrs = [],
           ...
           [#xmlel{name = Condition,
            ...
            #xmlel{name = <<"code">>,
               attrs = [],
               children = [{xmlcdata, Code}]}]}).

%%%----------------------------------------------------------------------
%%% API
%%%----------------------------------------------------------------------
start(SockData, Opts) ->
    ?GEN_FSM:start(ejabberd_wlan_c2s,
		   [SockData, Opts],
		   fsm_limit_opts(Opts) ++ ?FSMOPTS).

start_link(SockData, Opts) ->
    (?GEN_FSM):start_link(ejabberd_wlan_c2s,
			  [SockData, Opts],
			  fsm_limit_opts(Opts) ++ ?FSMOPTS).
...
%    FromJID = NewStateData#state.jid,
    ...
    FromJID = case catch fxml:get_attr_s(<<"type">>, Attrs) of
              <<"consult">> ->
    ...
    NewState = case ToJID of
		 ...
		 _ ->
		     case Name of
		       ...
		       <<"message">> ->
                           Packet = qtalk_public:send_recv_repley(NewEl,FromJID,ejabberd_wlan_c2s, NewStateData),
			   NewEl0 = ejabberd_hooks:run_fold(
				      user_send_packet, Server, NewEl,
				      [NewStateData, FromJID, ToJID]),
        %        Packet = NewEl0,
...
handle_info({kick, Code, _Xmlelement}, _StateName, StateData) ->
    ...
    ?DEBUG("Reason ~p ,Code ~p,Xml ~p  ~n",[Code,Reason,Xmlelement2]),
    catch send_element(StateData, Xmlelement2),
    {stop, normal,
...
send_element(StateData, El) ->
    NewEL = 
        case catch fxml:get_tag_attr_s(<<"chatid">>, El) of
        undefined ->
            fxml:replace_tag_attr(<<"chatid">>, <<"1">>, El);
        _ ->
            El
        end,
...
check_from(El, FromJID) ->
	...
	{value, SJID} ->
		...
		#jid{} ->
			...
			true ->
			 %   'invalid-from'
			 	qtalk_c2s:check_consult_from(El,FromJID,JID)
		    end
	    end
    end.
...
send_time_key_presence(Server,User,Resource,StateData) ->
    ...
    {ok,Key} ->
        ...
        NavPresence = qtalk_public:send_navversion(User,Server,Resource,<<"1003">>), 
        send_element(StateData,NavPresence),
