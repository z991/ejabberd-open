...
%%%----------------------------------------------------------------------
start(SockData, Opts) ->
    ?GEN_FSM:start(ejabberd_wlan_protobuf_c2s,
		   [SockData, Opts],
		   fsm_limit_opts(Opts) ++ ?FSMOPTS).

start_link(SockData, Opts) ->
    (?GEN_FSM):start_link(ejabberd_wlan_protobuf_c2s,
			  [SockData, Opts],
			  fsm_limit_opts(Opts) ++ ?FSMOPTS).
					  ...
					  fun(U, AuthzId, P) ->
						    ...
						    hosts ->
						    	ejabberd_auth:check_password_with_authmodule(
	                                                    U, AuthzId, Server, P);
                            _ ->
                                ejabberd_auth:check_wlan_password_with_authmodule(U, Server, P)
						    end
					    end,
...
session_established({xmlstreamend, _Name}, StateData) ->
    send_stream_end(StateData, 102, <<"established for xmlstreamend">>),
    {stop, normal, StateData};
...
%                 			send_time_key_presence(StateData#state.server,StateData#state.user,StateData#state.resource,StateData);
		       ...
		       <<"message">> ->
                           Packet = qtalk_public:send_recv_repley(NewEl,FromJID,ejabberd_wlan_protobuf_c2s, NewStateData),
			   NewEl0 = ejabberd_hooks:run_fold(
				      user_send_packet, Server, NewEl,
				      [NewStateData, FromJID, ToJID]),
%                Packet = NewEl0,
...
encode_iq_result_pb_packet(StateData,From,To, Packet) ->
    ...
    {<<"get_mac_push_notice">>,<<"http://jabber.org/protocol/mac_push_notice">>} ->
        ejabberd_xml2pb_iq:encode_pb_mac_push_notice_jid(From,To,Packet);
    {<<"mac_push_notice">>,<<"http://jabber.org/protocol/mac_push_notice">>} ->
        ejabberd_xml2pb_iq:encode_pb_mac_push_notice(From,To,Packet);
    {<<"calcel_mac_push_notice">>,<<"http://jabber.org/protocol/mac_push_notice">>} ->
        ejabberd_xml2pb_iq:encode_pb_cancel_mac_push_notice(From,To,Packet);
    _ ->
        case fxml:get_attr(<<"type">>, Packet#xmlel.attrs) of
        {value,<<"result">>} ->
