...
-export([
         gen_success_result/0,
         gen_success_result/1,
         gen_fail_result/2,
         gen_fail_result/3,
         cowboy_req_reply_json/2,
         check_ip/1,
         check_version/1,
         gen_result/3,
         gen_result/4,
...
gen_success_result(Content) ->
    gen_result(true, 0, <<"success">>, Content).

gen_fail_result(Code, Reason) ->
    gen_fail_result(Code, Reason, <<"">>).

gen_fail_result(Code, Reason, Content) ->
...
do_verify_user_key(_Server,_User,_Key) ->
	false.
